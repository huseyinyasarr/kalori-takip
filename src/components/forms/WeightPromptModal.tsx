...
const schema = z.object({
  weight: z.coerce.number().positive("Kilo pozitif sayı olmalı."),
});
...
export function WeightPromptModal({ profile }: { profile: UserProfile }) {
  ...
  const {
    ...
    defaultValues: { weight: profile.currentWeight || undefined },
  });
      ...
      <div className="w-full max-w-md rounded-lg bg-white p-5 shadow-soft">
        ...
        <form
          ...
          onSubmit={handleSubmit(async ({ weight }) => {
            await submitDailyWeight(user.uid, weight, profile.targetWeight);
          })}
        >
          <Input label="Güncel kilo" type="number" step="0.1" error={errors.weight?.message} {...register("weight")} />
